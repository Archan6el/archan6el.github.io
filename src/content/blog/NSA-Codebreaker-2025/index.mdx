...
```
~/dwarf2json/dwarf2json linux --elf vmlinux --system-map System.map > symtable.json
```
...
The ELF that it is searching through to find the section is itself. If we look at the call to `fill_in_DAT` again 

```c
fill_in_DAT(*param_2,__ptr,&DAT_001231c8,&local_170)
```

`*param_2` is what is passed in as the file to search through. This is being called in main, and `param_2` in main is the array `argv`, and `argv[0]` is the name of the executable itself, which is what the pointer `*param_2` is pointing to

Now the question is what is the section? Well, it's `.init.checksum.vjag~oel2`. 
...
The program it is extracting these sections from is itself, because if we look at the call to the function, similar to the `fill_in_DAT` function, it passes a pointer to `argv`, which is `argv[0]`. In otherwords, the name of the program itself

```c
if (extract_resource_from_file(*argv, r15_3, &var_180, &var_178) != 0)
```

So it appears that the malware has the section names obfuscated, it deobfuscates the name with the function we found, then extracts what's in that section from itself. The question is what does it do with the data now that it's extracted
...
Once the `zlib` decompression occurs, it takes the result, which is some sort of executable or shared library, and tries to run it in two ways based on a certain condition
...
In both above calls, `r15_3` is the result of the deobfuscation function call, storing the now deobfuscated string. Below is one example when deobfuscating the second obfuscated blob

```c
000030aa          *r15_3 = obfuscated_blob2
000030b7          *(r15_3 + 0xa) = obfuscated_blob2
000030b7          
000030c1          if (r14_1(&brainrot1, 0x50, r15_3, 0x1a) != 0)
```
