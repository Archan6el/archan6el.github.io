...
If we look at `DAT_001231c8` in Ghidra to see what the address is so that we can figure out what function it's pointing to, it's just all 0 bytes. This means it gets filled in at runtime
...
What this function is doing is that it is loading a named ELF section from a file, maps it into executable memory, and stores a function pointer to the start of that section. The function pointer in our case being `DAT_001231c8` 

We are only given this one binary, so we can assume that the ELF it is getting the section from is itself. Now the question is what is the section? Well, it's `.init.checksum.vjag~oel2`

Okay that's not entirely true, it's actally `.init.checksum.validation`, it appears that our deobfuscation wasn't completely perfect, but looking at the sections in Ghidra (or `readelf`), that's the only section that makes sense considering our partial deobfuscation. 

Now it may sound weird that we have a function pointer pointing to a section, as opposed to an actual function, but this actually works assuming that the section starts with a function. The function pointer, when invoked, will just execute that first function

Well, is there a function at `.init.checksum.validation`? 
...
Notice anything? They're essentially exactly the same. So this is why we couldn't find any references to the deobfuscation function we found. It's because the malware is assembling a function pointer to `.init.checksum.validation`, which contains the actual deobfuscation function it uses

This also explains why the first obfuscated blob was deobfuscated in `main`, instead of using the deobfuscation function. It's because the pointer to the deobfuscation function (or to the `.init.checksum.validation` section to be exact) hadn't been set up yet. After the function pointer is set up, the deobfuscation function is utilized for all subsequent use cases. 

Now another question that might arise is why have both. Why have a deobfuscation function for everyone to see, but is never invoked, and then have the exact same function hidden in `.init.checksum.validation`. Perhaps it was intentional by the devs to help static analyzers of this malware, or it was a compiler artifact, or perhaps it's because somewhere, the malware uses the non-hidden function to write to `.init.checksum.validation`
