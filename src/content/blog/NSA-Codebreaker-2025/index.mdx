...
Notice anything? They're exactly the same, essentially. So this is why we couldn't find any references to the deobfuscation function we found. It's because the malware is assembling a function pointer to `.init.checksum.validation`, which contains the actual deobfuscation function

Now another question that might arise is why have both. Why have a deobfuscation function for everyone to see, but is never invoked, and then have the exact same function hidden in `.init.checksum.validation`. Perhaps it was intentional to help static analyzers of this malware, or perhaps it's because somewhere, the malware uses the non-hidden function to write to `.init.checksum.validation`

Regardless of the reason, now we understand how exactly the malware is deobfuscating, and the purpose of the `.init.checksum.validation` section. Now, on to the next two. 
